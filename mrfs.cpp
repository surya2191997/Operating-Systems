...
int copy_myfs2pc(char* source, char* dest){
	fstream file(dest, ios::out|ios::binary);
	INode_t* cur_inode = &((INodeList_t*)(mrfs_mem + SB_SZ_B))->node[cur_dir];
	INode_t* node = get_file_inode(cur_inode, source);
	if(node == NULL){
		cout << "No file named " << source << " in current directory" << endl;
		return -1;
	}
	sem_wait(&mem_mutex);
	int cur_sz = node->sz;
	int idx = 0;
	while(cur_sz > 0){
		Block_t* b = (Block_t*)(mrfs_mem + DB_SZ * (*get_inode_ptr(node, idx)));
		file.write((char*)b, min(cur_sz, (int)sizeof(Block_t)));
		cur_sz -= min(cur_sz, (int)sizeof(Block_t));
		idx ++;
	}
	file.close();
	sem_post(&mem_mutex);
...
void print_perm(AccPer_t* perm){
	cout << (((perm->i >> 2)%2)?'r':'_') << (((perm->i >> 1)%2)?'w':'_') << (((perm->i)%2)?'x':'_');
	cout << (((perm->g >> 2)%2)?'r':'_') << (((perm->g >> 1)%2)?'w':'_') << (((perm->g)%2)?'x':'_');
	cout << (((perm->o >> 2)%2)?'r':'_') << (((perm->o >> 1)%2)?'w':'_') << (((perm->o)%2)?'x':'_');
}
	...
	while(rem_count){
		...
		for(int i=0;i<FILES_PER_DIR;i++){
			if(dir->entry[i].ptr != -1){
				...
				INode_t* f = &((INodeList_t*)(mrfs_mem + SB_SZ_B))->node[dir->entry[i].ptr];
				cout << f->sz << "\t " << ((f->ftype == 0)?"F":"D") << "\t ";
				char* t = asctime(gmtime(&f->last_modified));
				t[strlen(t)-1] = '\0';
				cout << t << "\t ";
				t = asctime(gmtime(&f->last_read));
				t[strlen(t)-1] = '\0';
				cout << t << "\t ";
				print_perm(&f->permissions);
				cout << endl;
				rem_count --;
			}
		}
...
int dump_myfs(char *dumpfile){
	fstream file(dumpfile, ios::out|ios::binary);
	if(!file.is_open())
		return -1;
	sem_wait(&mem_mutex);
	int sz = ((SuperBlock_t*)(mrfs_mem))->total_sz;
	file.write((char*)&sz, sizeof(sz));
	file.write(mrfs_mem, sz);
	file.close();
	sem_post(&mem_mutex);
	return 0;
}
...
int restore_myfs(char *dumpfile){
	fstream file(dumpfile, ios::in|ios::binary);
	init_sems();
	if(!file.is_open())
		return -1;
	sem_wait(&mem_mutex);
	...
	mrfs_mem = new char[sz];
	file.read(mrfs_mem, sz);
	cur_dir = 0;
	init_fdt();
	file.close();
	sem_post(&mem_mutex);
...
int status_myfs(){
	...
	SuperBlock_t* sb = (SuperBlock_t*)mrfs_mem;
	cout << "\t\t***** SUMMARY *****" << endl;
	cout << "Total size: " << sb->total_sz << endl;
	cout << "Free size: " << ((sb->max_db - sb->cur_db)*DB_SZ) << endl;
	cout << "No of files and directories: " << sb->cur_inodes << endl;
	sem_post(&mem_mutex);
	return 0;
}
...
int chmod_myfs(char* name, int mode){
	INode_t* cur_inode = &((INodeList_t*)(mrfs_mem + SB_SZ_B))->node[cur_dir];
	INode_t* fnode = get_file_inode(cur_inode, name);
	if(fnode == NULL)
		return -1;
	sem_wait(&mem_mutex);
	fnode->permissions = {(unsigned char)((mode >> 6)%8), (unsigned char)((mode >> 3)%8), (unsigned char)((mode)%8)};
	sem_post(&mem_mutex);
	return 0;
}
